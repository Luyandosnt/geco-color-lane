...
export class ColorLane extends Phaser.Scene {
  ...
  title!: Phaser.GameObjects.Text; hint!: Phaser.GameObjects.Text; playText!: Phaser.GameObjects.Text;
  lanes = [95,195,295];
  ...
  create() {
    ...
    this.playText=this.add.text(195,570,'TAP TO PLAY',{fontFamily:'Arial',fontSize:'24px',color:'#fde047',fontStyle:'bold'}).setOrigin(.5);
  ...
  start(){ this.playing=true; this.score=0; this.runCoins=0; this.speed=250; this.scoreText.setText('0'); this.title.setVisible(false);this.hint.setVisible(false);this.playText.setVisible(false);Core.event('run_start');this.time.addEvent({delay:900,loop:true,callback:()=>this.spawn()}); }
  ...
  update(_:number,delta:number){ if(!this.playing)return; this.speed=Math.min(600,250+this.score*6); for(const obj of this.gates.getChildren() as Gate[]){ obj.y+=this.speed*delta/1000; if(obj.y>680&&obj.y<755&&!obj.getData('checked')){obj.setData('checked',true);if(obj.lane===this.lane){if(obj.colorIndex===this.lane){this.score++;this.runCoins++;this.scoreText.setText(String(this.score));}else{return this.over();}}} if(obj.y>880)obj.destroy(); } }
  over(){ if(!this.playing)return;this.playing=false;this.time.removeAllEvents();this.gates.clear(true,true);Core.finish(this.score,this.runCoins);Core.event('run_end',{score:this.score});this.title.setText('GAME OVER').setVisible(true);this.hint.setText('SCORE '+this.score+'\nBEST '+Core.data.highScore+'\nCOINS '+Core.data.coins+'\n\nTap to play again').setVisible(true);this.playText.setText('TAP TO RETRY').setVisible(true); }
}
